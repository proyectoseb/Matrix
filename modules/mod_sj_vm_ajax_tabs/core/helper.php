...
abstract class VmAjaxtabsHelper extends VmAjaxtabsBaseHelper
{
	...
	public static function getList($params, $module)
	{
		...
		} else if (isset($catid_preload) && in_array($catid_preload, $catids)) {
			$catid_loadfirst = (int)$catid_preload;
		}

		$_list = self::_getCategoryInfor($catids, $params);
		if (empty($_list)) return;

		foreach ($_list as $item) {
			if ($item->virtuemart_category_id == $catid_loadfirst) {
				$item->child = self::_getProductInfor($catid_loadfirst, $params);
			}
			$list[$item->virtuemart_category_id] = $item;
		}
		return $list;
	}
	...
	public static function _getCategoryInfor($catids, $params)
	{
		...
		foreach ($list as $catid) {
			...
			$catid->count = self::_CountProduct($catid->virtuemart_category_id, $params);
