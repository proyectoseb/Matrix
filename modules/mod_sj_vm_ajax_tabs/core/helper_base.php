<?php
/**
 * @package Sj Ajax Tabs for VirtueMart
 * @version 1.0.0
...
if (!class_exists('VmAjaxtabsBaseHelper')) {
	/**
	 * BaseHelper for com_content only.
	 */
	abstract class VmAjaxtabsBaseHelper
	{
		/**
			...
			if (!isset(self::$image_cache[$hash][$item->id])) {
				...
				if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
					if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
						$images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_categories_accordion/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'respl-placeholder');
					} else {
						$images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_categories_accordion/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'placeholder');
					}
				}
