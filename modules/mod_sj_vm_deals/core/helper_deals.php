...
abstract class VMDealsHelper extends VMDealsBaseHelper {
	...
	public static function getList(&$params,$getC = 0)
	{
		...
		if(!empty($catids)){
			...
			$productModel = new VirtuemartModelProductADExtend();
...
if(!class_exists('VirtuemartModelProductADExtend') && class_exists('VirtueMartModelProduct')){
	class VirtuemartModelProductADExtend extends VirtueMartModelProduct{
		function sortSearchListQuery ($onlyPublished = TRUE, $virtuemart_category_id = FALSE, $group = FALSE, $nbrReturnProducts = FALSE, $langFields = Array()) {
			...
			if (count ($where) > 0) {
				$whereString = ' WHERE pp.product_price_publish_up <= "'.$date.'" AND pp.product_price_publish_down > "'.$date.'" AND (' . implode (' AND ', $where) . ') ';
			}
			else {
				$whereString = '';
			}
