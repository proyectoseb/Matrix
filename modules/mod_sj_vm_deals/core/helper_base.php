<?php
/**
 * @package SJ Listing Tabs for VirtueMart
 * @version 1.0.0
...
if (!class_exists('VMDealsBaseHelper')) {
    /**
     * BaseHelper for com_content only.
     */
    abstract class VMDealsBaseHelper
    {
        /**
            ...
            if (!isset(self::$image_cache[$hash][$item->id])) {
                ...
                if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
                    if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_listing_tabs/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'respl-placeholder');
                    } else {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_listing_tabs/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'placeholder');
                    }
                }
        ...
        public static function getVmCImage($item, $params, $prefix = 'imgcfgcat')
        {
            $images = self::getVmCImages($item, $params, $prefix);

            return is_array($images) && count($images) ? $images[0] : null;
        }

        public static function getVmCImages($item, $params, $prefix = 'imgcfgcat')
        {
            $hash = md5(serialize(array($params, 'category')));
            if (!isset(self::$image_cache[$hash][$item->virtuemart_category_id])) {
                $defaults = array(
                    'external' => 1,
                    'image_category' => 1,
                    'description' => 1
                    ...
                    foreach ($mark as $type => $true) {
                        switch ($type) {
                            case 'image_category':
                                ...
                                if (!empty($image_path) && file_exists($image_path) && @getimagesize($image_path) !== false) {
                                    $image = array(
                                        'src' => $image_path
                                    );
                                    $image['alt'] = $item->category_name;
                                    $image['title'] = $item->category_name;
                                    array_push($images_path, $image);
                                }
                                break;
                            case 'description':
                                if ($type == 'description') {
                                    $text = $item->category_description;
                                }
                                $inline_images = self::getInlineImages($text);
                                for ($i = 0; $i < count($inline_images); $i++) {
                                    array_push($images_path, $inline_images[$i]);
                                ...
                                break;
                            case 'external':
                                $exf = $params->get($prefix . '_external_url', 'images/vm/{name}/');
                                preg_match_all('/{([a-zA-Z0-9_]+)}/', $exf, $m);
                ...
                self::$image_cache[$hash][$item->virtuemart_category_id] = $images_path;

            }
            return self::$image_cache[$hash][$item->virtuemart_category_id];
        }
