...
if (!class_exists('VmCategoriesBaseHelper')) {
    /**
     * BaseHelper for com_content only.
     */
    abstract class VmCategoriesBaseHelper
    {
        /**
            ...
            if (!isset(self::$image_cache[$hash][$item->id])) {
                ...
                if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
                    if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_categories/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'respl-placeholder');
                    } else {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_categories/assets/images/nophoto.jpg'), 'title' => $item->title, 'alt' => $item->title, 'class' => 'placeholder');
                    }
                }
                        ...
                        switch ($type) {
                            ...
                            case 'image_category':
                                ...
                                if (!empty($image_path) && file_exists($image_path) && @getimagesize($image_path) !== false) {
                                    ...
                                    //var_dump($item->category_name);die();
