...
abstract class SjVMSuperCategoriesHelper extends SjVMSuperCategoriesHelperBaseHelper
{	
    ...
    public static function getList($params)
    {
		...
		$return = array();
		$return['category_parent'] = self::_getCategoryInfor($_catids, $params);		
        $catids = self::_getChildCategories($_catids, $params);
        $list = array();
		$return['category_tree'] = self::_getCategoryInfor($catids, $params);
        ...
        foreach ($filters as $filter) {
			...
			$product->countp = self::_CountProduct('*', $params, $filter);
        ...
        foreach ($products as $product) {
			if ($product->countp > 0) {
                 if ($product->virtuemart_category_id == $filter_preload) {
                    ...
                    $product->title = $product->virtuemart_category_id;
                }
				$product->_odering = $product->virtuemart_category_id;
				$product->title = self::getLabel($product->virtuemart_category_id);
    ...
    public static function _getCategoryInfor($catids, $params)
    {
        ...
        if ($cat_ordering_direction == 'ASC') {
            switch ($cat_order_by) {
                default:
                case 'title':
                    usort($return, create_function('$a, $b', 'return strnatcasecmp( $a->category_name, $b->category_name);'));
                    break;
                case 'created':
                    usort($return, create_function('$a, $b', 'return $a->created_on < $b->created_on;'));
                    break;
                case 'random':
                    shuffle($return);
                    break;
            ...
            switch ($cat_order_by) {
                default:
                case 'title':
                    usort($return, create_function('$a, $b', 'return strnatcasecmp( $b->category_name, $a->category_name);'));
                    break;
                case 'created':
                    usort($return, create_function('$a, $b', 'return $a->created_on > $b->created_on;'));
                    break;
    ...
    public static function _CountProduct($catids, $params, $field_order)
    {
        ...
        if (!empty($catids)) {
            ...
            $productModel = new VirtuemartModelProductExtendS();
    ...
    public static function _getProductInforLoadMore($catids, $params, $field_order = null,$limit_start,$limit,$orderdir)
    {
		VmConfig::loadJLang('com_virtuemart', true);
		VmConfig::loadConfig();
        if ($catids == '*') {
        ...
        if (!empty($catids)) {
            ...
            $productModel->filter_order = $field_order;
            $productModel->specail_product = $p_special;
            $productModel->ordering_direction = $orderdir;
            $productModel->_limitStart = $limit_start;
...
if (!class_exists('VirtuemartModelProductExtendS') && class_exists('VirtueMartModelProduct')) {
    class VirtuemartModelProductExtendS extends VirtueMartModelProduct
    {
        ...
        public function _exeSortSearchListQuery($object, $select, $joinedTables, $whereString = '', $groupBy = '', $orderBy = '', $filter_order_Dir = '', $nbrReturnProducts = false){
            ...
            if(isset($this->_withCount) && $this->_withCount){
