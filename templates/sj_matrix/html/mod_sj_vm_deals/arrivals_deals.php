 ...
 */
	...
	if($params->get('item_first_product_display',1) == 1){
		...
		'type' => $params->get('imgcfg_f_type'),
		'width' => $params->get('imgcfg_f_width'),
		'height' => $params->get('imgcfg_f_height'),
		'quality' => 90,
		'function' => ($params->get('imgcfg_f_function') == 'none') ? null : 'resize',
		'function_mode' => ($params->get('imgcfg_f_function') == 'none') ? null : substr($params->get('imgcfg_f_function'), 7),
		'transparency' => $params->get('imgcfg_f_transparency', 1) ? true : false,
		'background' => $params->get('imgcfg_f_background'));
		echo '<div class="sj_vm_deals_first_product">';
			echo '<div class="item">';
				...
				echo '<div class="item-inner">';
					$item_img = VMDealsHelper::getVmImage($list[0], $params, 'imgcfg_f');
					if ($item_img) {
					...
					if (($params->get('item_title_display', 1) == 1) || ($params->get('item_rating_display', 1) == 1) || (int)$params->get('item_prices_display', 1)){
						echo '<div class="product-info">';
							echo '<div class="block-ratting-title">';
							...
							if($params->get('item_popup_display') == 1){
									...
									if ($params->get('item_quickview_display', 1) == 1) {
										echo '<div class="item-quick-view">';
											echo '<a href="'.$list[0]->link.'" title="'.$list[0]->title.'" '.VMDealsHelper::parseTarget($params->get('link_target')).' style="display:none;">';  
										echo '</div>';
									} 
